...
unsigned long isBuzzerTime;
    if (fPUMP) {
        switch(buzzerState) {
            ...
            case false:
                isIntervalTime = (now-previousBuzzerTime)>=buzzer_interval;
                if (isIntervalTime) {
                    previousBuzzerTime += buzzer_interval;
                //     // previousBuzzerTime = now;
                //     lnprint(fPrint_BEEP, "phase: ", phase, "");
...
// ==================================
void checkPassiveBuzzer() {
    if (isBuzzerTime!=0 && isBuzzerTime<now) { // se stiamo suonando ed è scaduto il time... chiudiamo il buzzer
...
void checkPumpState() {
    ...
    switch(fPUMP) {
        case ON:
            fHORN=true;
            buzzerState=true;
            if (isBuzzerTime!=0) break;
            if (isIntervalTime!=0) break;
            // buzzer_cycle=true;
            if (phase>=5)  {
